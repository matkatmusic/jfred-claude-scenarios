...
def check_quantity(item, n):
    ...
    Examples:
        >>> check_quantity({"name": "bolt", "stock": 50}, 10)
        True
        >>> check_quantity({"name": "bolt", "stock": 3}, 10)
        False
    """
    return n <= item["stock"]
...
def insert_item(store, item):
    ...
    Examples:
        >>> s = []
        >>> insert_item(s, {"name": "nut", "price": 0.10, "stock": 200})
        >>> len(s)
        1
    """
    store.append(item)
...
def low_stock(store, threshold):
    ...
    Args:
        store:     list of item dicts.
        threshold: int — items with ``stock < threshold`` are returned.

    Returns:
        list of item dicts with stock strictly less than *threshold*.
    """
