...
def order_line(item):
    ...
    total = fmt_money(item["price"] * item["qty"])
    in_stock = "In Stock" if chk_stock(item, item["qty"]) else "Low Stock"
    return f"{item['name']}: {item['qty']} x {fmt_money(item['price'])} = {total} [{in_stock}]"
