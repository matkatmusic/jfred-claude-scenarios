...
def release(store, name, n):
    """Return *n* reserved units of *name* back to available stock.
    ...
    Raises:
        KeyError: If no item with that name is found.
        ValueError: If *n* exceeds the current reserved count.
    """
    item = find_item(store, name)
    if item is None:
        raise KeyError(f"Item '{name}' not found")
    reserved = item.get("reserved", 0)
    if n > reserved:
        raise ValueError(f"Cannot release {n}; only {reserved} reserved")
    item["reserved"] = reserved - n


def audit(store):
    """Return a summary dict of the entire store.

    Args:
        store: List of item dicts.

    Returns:
        Dict with ``"item_count"``, ``"total_units"``, and ``"total_value"`` keys.
    """
    return {
        "item_count": len(store),
        "total_units": sum(item["stock"] for item in store),
        "total_value": tot_value(store),
    }
