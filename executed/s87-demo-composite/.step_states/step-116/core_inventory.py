...
def stock_of(store, name):
    """Return the stock count for the item named *name*.
    ...
    Returns:
        The item's ``"stock"`` value (int), or 0 if not found.
    """
    item = lookup_item(store, name)
    return item["stock"] if item is not None else 0


def transfer_stock(store, src, dst, n):
    """Move *n* units of stock from item *src* to item *dst*.

    Both items must already exist in *store*. The source item's stock is
    decreased by *n* and the destination item's stock is increased by *n*.

    Args:
        store: List of item dicts (mutated in place).
        src: Name of the item to take stock from.
        dst: Name of the item to receive stock.
        n: Number of units to transfer.

    Raises:
        KeyError: If *src* or *dst* is not found in *store*.
        ValueError: If *src* has fewer than *n* units available.
    """
    src_item = lookup_item(store, src)
    if src_item is None:
        raise KeyError(f"Item '{src}' not found")
    dst_item = lookup_item(store, dst)
    if dst_item is None:
        raise KeyError(f"Item '{dst}' not found")
    if not qty_chk(src_item, n):
        raise ValueError(f"Cannot transfer {n}; only {src_item['stock']} available")
    src_item["stock"] -= n
    dst_item["stock"] += n


def merge_stores(a, b):
    """Combine two stores into a new store.

    Items unique to either store are copied. Items present in both (matched
    by name) are merged: stock is summed and the higher price is kept.

    Args:
        a: First list of item dicts.
        b: Second list of item dicts.

    Returns:
        A new list of item dicts containing the merged inventory.
    """
    merged = {item["name"]: dict(item) for item in a}
    for item in b:
        name = item["name"]
        if name in merged:
            merged[name]["stock"] += item["stock"]
...
if __name__ == "__main__":
    ...
    assert lookup_item(store, "Gadget") is None
    assert len(store) == 2
...
# checked-2
def snapshot(store):
    """Return a shallow copy of *store* with each item dict copied.

    Args:
        store: List of item dicts.

    Returns:
        A new list of new dicts, so mutations to the copy do not affect the original.
    """
    return [dict(item) for item in store]
