...
def transfer_stock(store, src, dst, n):
    ...
    """
    src_item = find_item(store, src)
    if src_item is None:
        raise KeyError(f"Item '{src}' not found")
    dst_item = find_item(store, dst)
    if dst_item is None:
...
def reset_store(store):
    """Remove all items from *store* and return them.

    Args:
        store: List of item dicts (mutated in place — emptied).

    Returns:
        List of the item dicts that were removed.
    """
    items = list(store)
    store.clear()
    return items
...
def is_empty(store):
    """Check whether *store* contains no items.
