...
def shrink(store, name):
    """Remove item `name` from `store` via remove_item. Returns True if removed."""
    try:
...
if __name__ == "__main__":
    ...
    insert_item(s, {"name": "rivet", "price": 0.10, "stock": 50})
    assert shrink(s, "rivet") is True
    assert find_item(s, "rivet") is None
    assert shrink(s, "rivet") is False
