...
def report(book):
    """Record a snapshot entry and return total debits.

    Adds a 'report_snapshot' debit entry of 0 to mark that a report
    was generated, then returns the total debits.

    Args:
        book: list of entry dicts.

    Returns:
        float: total debit amount after snapshot entry.
    """
    record_entry(book, {"name": "report_snapshot", "type": "debit", "amount": 0})
    return total_debits(book)
