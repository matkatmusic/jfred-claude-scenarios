...
def reorder(store, name, n):
    # ponytail: near-duplicate of restock — consider merging if both survive
    item = find_item(store, name)
    if item is None or not check_quantity(item, n):
        insert_item(store, {"name": name, "price": 0.0, "stock": n})


def shrink(store, name):
    # ponytail: thin wrapper over remove_item — justify or merge
