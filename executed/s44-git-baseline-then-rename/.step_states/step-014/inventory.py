...
def restock(store, name, n):
    """Add *n* units of *name* to *store*, inserting it if absent."""
    item = find_item(store, name)
    if item and check_quantity(item, 0):
        item["stock"] += n
    else:
...
def reorder(store, name, n):
    """Reorder *n* units of *name* — only if stock is insufficient."""
    item = find_item(store, name)
...
def shrink(store, name):
    """Remove *name* from *store* and return it."""
    return remove_item(store, name)
