...
def mk_order(items):
    ...
    Returns:
        dict with keys:
            ...
            - "subtotal": result of calculate_total
            - "subtotal_fmt": formatted subtotal string
            - "all_in_stock": bool, True if every item passes check_stock
    """
    subtotal = calculate_total(items)
    return {
        "items": items,
        "subtotal": subtotal,
        "subtotal_fmt": format_currency(subtotal),
        "all_in_stock": all(check_stock(i, i["qty"]) for i in items),
    }
