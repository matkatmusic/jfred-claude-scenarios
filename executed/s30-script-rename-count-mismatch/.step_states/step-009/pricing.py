...
def receipt(item: dict[str, Any]) -> str:
    """One-line receipt string: name and rounded price.

    >>> receipt({"name": "widget", "price": 9.999})
    'widget: 10.0'
    """
    return f"{item.get('name', 'item')}: {round_to_cents(base_price(item))}"
