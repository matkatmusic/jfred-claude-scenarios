...
insert_item    Append an item to a store.
remove_item     Remove an item from a store by name.
...
-------
    >>> store = []
    >>> insert_item(store, {"name": "bolt", "price": 0.25, "stock": 500})
    >>> check_quantity(store[0], 100)
    True
    >>> tot_value(store)
    125.0
"""
...
def low_stock(store, threshold):
    ...
    ----------
    store : list[dict]
        Each element must have a ``"stock"`` key.
    threshold : int
        Items with ``stock < threshold`` are included.
...
def restock(store, name, n):
    """Ensure at least *n* units of *name* are in stock.

    If the item exists and already has enough stock (per ``check_quantity``),
    this is a no-op. If stock is insufficient, it is topped up to *n*.
    If the item does not exist, a new entry is inserted via ``insert_item``
    with a price of 0 (caller should update).
    ...
    if item is not None:
        ...
        insert_item(store, {"name": name, "price": 0, "stock": n})
