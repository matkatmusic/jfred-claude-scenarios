...
class TestQtyChk:
    def test_exact_stock(self):
        assert check_quantity(_item(stock=5), 5) is True

    def test_under_stock(self):
        assert check_quantity(_item(stock=5), 3) is True

    def test_over_stock(self):
        assert check_quantity(_item(stock=5), 6) is False

    def test_zero_requested(self):
        assert check_quantity(_item(stock=0), 0) is True

    def test_negative_raises(self):
        with pytest.raises(ValueError):
            check_quantity(_item(), -1)
