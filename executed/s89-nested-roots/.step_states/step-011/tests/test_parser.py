from parser import parse_line, count_tokens, strip_comment
...
class TestStripComment:
    def test_trailing_comment(self):
        """Trailing '#' comment is removed, code portion preserved."""
        assert strip_comment("x = 1  # assign") == "x = 1"


class TestIntegration:
    def test_strip_then_parse(self):
        """Stripping a comment then parsing yields only the code tokens."""
        cleaned = strip_comment("x = 1  # assign")
        result = parse_line(cleaned)
        assert result["words"] == ["x", "=", "1"]
