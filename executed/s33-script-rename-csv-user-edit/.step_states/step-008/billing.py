...
Functions:
    calculate_total(items)        Sum line items (price * qty).
    format_currency(n)           Format a number as "$0.00".
    check_stock(item, qty)   Check if requested qty is in stock.
    apply_discount(total, pct) Apply a percentage discount.
"""
...
def check_stock(item: dict, qty: int) -> bool:
...
def reorder(item: dict, qty: int) -> float:
    """Place a reorder if stock is sufficient.

    Args:
        item: Dict with 'price', 'qty', and 'stock' keys.
        qty: Quantity to reorder.

    Returns:
        Order total for the reorder line.

    Raises:
        ValueError: If requested qty exceeds stock.
    """
    if not check_stock(item, qty):
        raise ValueError(f"Insufficient stock: requested {qty}, have {item['stock']}")
    return calculate_total([{"price": item["price"], "qty": qty}])
...
if __name__ == "__main__":
    ...
    assert calculate_total(sample) == 25.50
    assert calculate_total([]) == 0.0

    assert format_currency(25.50) == "$25.50"
    assert format_currency(0) == "$0.00"
    assert format_currency(-5) == "-$5.00"
    assert format_currency(1000.1) == "$1000.10"

    assert check_stock({"stock": 10}, 10) is True
    assert check_stock({"stock": 10}, 11) is False
    assert check_stock({"stock": 0}, 0) is True
